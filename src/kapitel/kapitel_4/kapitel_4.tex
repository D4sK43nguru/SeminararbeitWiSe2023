...
Besonders gegen \ac{ddos}-Angriffe bietet eine \ac{zta} starken Schutz, da die häufig automatisierten Angriffe durch die Mikrosegmentierung nur geringe Bereiche des Systems anwählen können.\autocite[\vglf][\pagef 289]{Eidle-2017}
...
So wurden zum Beispiel allein im Jahr $2018$ ungefähr $16.500$ Einträge der \ac{nvd} der USA hinzugefügt, im Jahr $2022$ bereits ungefähr $25.200$ \autocites[\vglf][\pagef 6]{cunningham-2019}[\vglf][]{cve-2023}.
...
Da eine \ac{zta} auch in bestehende Systeme partiell eingeführt werden kann, ist dies kein äußerst Hindernis.

\subsection{Auswirkungen auf die Produktivität}\label{subsec:auswirkungen-auf-die-produktivitat}
Wie jede Sicherheitsmaßnahme in der Informationstechnik stellt auch \ac{zt} einen Eingriff in die Nutzerproduktivität dar.
So stellten 2021 für ungefähr $22\%$ der Arbeitnehmer in Europa und den USA, die Remote Work betrieben, komplexe Passwörter und \ac{mfa}-Anforderungen und für ungefähr $25\%$ der Arbeitnehmer ein Mangel an \ac{SSO}-Möglichkeiten Herausforderungen dar.\autocite[\vglf][\pagef 27]{citrix-2021}
...
Jedoch können die \glsdisp{authentifizierung}{Authentifizierungsanforderungen} an die Nutzer auch deutlich komplexer gestaltet werden, wie \zb mit \ac{mfa}-Anforderungen, oder \glsdisp{captcha}{\acs{captcha}}.

Während diese erweiterten Anforderungen die \glsdisp{authentifizierung}{Authentizität} des Nutzers sicherstellen, so sind sie für den Nutzer zunächst ein Eingriff in seine Produktivität und den Arbeitsfluss.
Dadurch ist es notwendig, bei der \glsdisp{authentifizierung}{Nutzerauthentifizierung} eine Balance zwischen Sicherheit und Produktivität zu finden\autocite[\vglf][\pagef 7]{pace-2004}.